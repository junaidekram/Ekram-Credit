...
function startTransactionFlow(dollarAmount, selectedUser) {
    ...
    const transactionResult = processTokenConversion(dollarAmount, selectedUser);
    ...
    }, 12000); // 2 more seconds for "Remove Card"
    
    // Return to dashboard after completion
    setTimeout(() => {
        window.location.href = 'dashboard.html';
    }, 15000); // 3 more seconds for "Transaction Complete"
}
...
function updateDashboardData() {
    ...
    rows.forEach(row => {
        ...
        if (tokenCell && amountDueCell) {
            ...
            // Update amount due
            const amountDue = userAmounts[username] || 0;
            if (amountDue > 0) {
